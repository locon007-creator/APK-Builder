...
public final class LocalZipBuildEngine {
  ...
  public static BuildEvidence build(
      Path templateApk, String contractJson, String builderVersion, Path attemptsRoot, Path projectArchive,
      PrivateKey signingKey, X509Certificate signingCertificate, Path verifiedOutput) throws TemplateException {
    TemplateContract contract = TemplateContractParser.parse(contractJson);
    return build(templateApk, contractJson, builderVersion, attemptsRoot, projectArchive,
  ...
  public static BuildEvidence build(
      Path templateApk, String contractJson, String builderVersion, Path attemptsRoot, Path projectArchive,
      AppIdentity identity, PrivateKey signingKey, X509Certificate signingCertificate, Path verifiedOutput) throws TemplateException {
    ...
    try {
      ...
      ApkProjectInjector.injectProject(staged, contentApk, project.projectRoot(), project.entryFile(), identity);
      ...
      verifyPayload(signed, project, contract, identity);

      String sourceAfter = Hashing.sha256(templateApk);
      if (!sourceBefore.equals(sourceAfter)) throw new TemplateException(TemplateErrorCode.OUTPUT_VERIFY_FAILED, "Immutable bundled template changed during ZIP build");
      ...
      AtomicApkPublisher.publish(signed, verifiedOutput, candidate -> {
        ...
        verifyPayload(candidate, project, contract, identity);
      });
  ...
  private static void verifyPayload(Path signed, ProjectArchive.PreparedProject project, TemplateContract contract, AppIdentity identity) throws Exception {
    try (ZipFile zip = new ZipFile(signed.toFile())) {
      ...
      for (String relativeName : project.files()) {
        ...
        if (!source.startsWith(project.projectRoot()) || !Files.isRegularFile(source, LinkOption.NOFOLLOW_LINKS)) throw new TemplateException(TemplateErrorCode.OUTPUT_VERIFY_FAILED, "Prepared project file is missing: " + relativeName);
        String zipName = "assets/html/" + relativeName.replace('\\', '/');
        expectedAssets.add(zipName);
        ZipEntry entry = zip.getEntry(zipName);
        if (entry == null || entry.isDirectory()) throw new TemplateException(TemplateErrorCode.OUTPUT_VERIFY_FAILED, "Output missing project file: " + relativeName);
        byte[] expected = Files.readAllBytes(source);
        byte[] actual; try (InputStream in = zip.getInputStream(entry)) { actual = in.readAllBytes(); }
        if (!Arrays.equals(expected, actual)) throw new TemplateException(TemplateErrorCode.OUTPUT_VERIFY_FAILED, "Output project file differs from input: " + relativeName);
      }
      Set<String> actualAssets = new TreeSet<>();
      Enumeration<? extends ZipEntry> entries = zip.entries();
      while (entries.hasMoreElements()) { ZipEntry entry = entries.nextElement(); if (!entry.isDirectory() && entry.getName().startsWith("assets/html/")) actualAssets.add(entry.getName()); }
      if (!actualAssets.equals(expectedAssets)) throw new TemplateException(TemplateErrorCode.OUTPUT_VERIFY_FAILED, "Output HTML project file set does not match input");
      ZipEntry configEntry = zip.getEntry("assets/app_config.json");
      if (configEntry == null || configEntry.isDirectory()) throw new TemplateException(TemplateErrorCode.OUTPUT_VERIFY_FAILED, "Output missing shell config");
      String config; try (InputStream in = zip.getInputStream(configEntry)) { config = new String(in.readAllBytes(), java.nio.charset.StandardCharsets.UTF_8); }
      if (!config.contains("\"appType\":\"HTML\"") || !config.contains("\"siteAssetBase\":\"html\"") ||
          !config.contains("\"entryFile\":\"" + jsonEscape(project.entryFile()) + "\"") || !config.contains("\"enableNativeBridge\":false") ||
          !config.contains("\"packageName\":\"" + jsonEscape(identity.packageName()) + "\"") ||
          !config.contains("\"appName\":\"" + jsonEscape(identity.appName()) + "\"")) {
